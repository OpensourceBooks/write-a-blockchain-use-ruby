class B
    ...
    def init(data)
        ...
        pre_hash = 0
        hash = h(index,timestamp,data,pre_hash)
    ...
    def v
        ...
        blockchain.each do |block|
            ...
            if h(block[:index],block[:timestamp],block[:data],block[:pre_hash]) != block[:hash]
                @flag = false 
            end
            ...
            if @first
                # 从创世区块后开始对比 pre_hash 
                @first = false
                @last_hash = h(block[:index],block[:timestamp],block[:data],block[:pre_hash])
            else 
